...
use core::mem;
use core::ops::Deref;
use ffi::{types::AlignedType, CPtr};
...
impl<C: Signing> Secp256k1<C> {
    ...
    #[inline]
    #[cfg(any(test, feature = "rand"))]
    pub fn generate_keypair(&self) -> (key::SecretKey, key::PublicKey) {
        let sk = key::SecretKey::new();
...
mod tests {
    ...
    use super::{Message, Secp256k1, Signature};
    use context::*;
    ...
    fn test_manual_create_destroy() {
        ...
        let size = 0;
        let full: Secp256k1<AllPreallocated> = Secp256k1 {
            ctx: ctx_full,
            phantom: PhantomData,
            size,
        };
        let sign: Secp256k1<SignOnlyPreallocated> = Secp256k1 {
            ctx: ctx_sign,
            phantom: PhantomData,
            size,
        };
        let vrfy: Secp256k1<VerifyOnlyPreallocated> = Secp256k1 {
            ctx: ctx_vrfy,
            phantom: PhantomData,
            size,
        };

        let (sk, pk) = full.generate_keypair();
    ...
    fn test_panic_raw_ctx_should_terminate_abnormally() {
        ...
        raw_ctx_verify_as_full.generate_keypair();
    }
